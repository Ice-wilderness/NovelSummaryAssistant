          ...
          <>
            ...
            <div className="rule-group-list">
              {profileDraft.rule_groups.map((group) => {
                const rules = profileDraft.rules.filter((rule) => rule.group_id === group.id);
                return (
                  <section className="rule-group-panel" key={group.id}>
                    ...
                    {rules.length === 0 ? (
                      <span className="empty-state">此分组暂无规则</span>
                    ) : (
