...
- 为异常项目生成 repair plan，区分仅需校正 metadata/index/path 的无 LLM 修复、需要调用 LLM 补齐总结内容的修复、缺少源文件/配置无法修复、存在覆盖风险等情况。
- 提供用户显式触发的修复/续跑任务：只在用户确认后校正状态、补齐缺失输出或重新生成缺失阶段；任何总结内容补齐都按可能调用 LLM 处理，涉及 LLM 调用、覆盖已有文件或结果可能变化时必须提前披露。
