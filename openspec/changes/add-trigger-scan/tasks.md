...
- [ ] 7.2 Implement scan startup validation for project type, chapter files, selected profile, selected APIs, scan range, scan mode, summary coverage, migration requirement, and resumable state.
- [ ] 7.3 Implement hybrid coarse scan over small-summary batches with configurable batch size.
- [ ] 7.4 Implement precise scan over paragraph-indexed chapter text with strict finding schema validation.
...
- [ ] 7.6 Implement optional verification with same-chapter finding batching and independent verification API selection.
...
- [ ] 7.9 Implement resume logic that skips completed chapters when configuration and profile version are compatible.
- [ ] 7.10 Add tests for prechecks, coarse scan parsing, precise scan parsing, verification application, aggregation, thresholds, resume, cancel preservation, and invalid model output handling.
...
- [ ] 10.3 Add trigger scan precheck, start, status, report, result action, skip list, export, and context APIs.
...
- [ ] 11.3 Build scan configuration tab with project selector, range controls, mode selection, API selection, confidence controls, advanced settings, and startup-check decision dialogs.
...
- [ ] 11.10 Add frontend tests or documented manual checks for tab navigation, configuration validation, precheck decisions, live updates, filters, spoiler controls, context modal, review actions, and exports.
...
- [ ] 12.3 Update project import/status scanning to recognize trigger scan reports, skip lists, paragraph cache, and migration requirements.
- [ ] 12.4 Add tests for managed output cleanup, custom output preservation, imported trigger scan history, and migration status display.
- [ ] 12.5 Commit this feature block after the focused checks pass.

## 13. End-To-End Verification

- [ ] 13.1 Run focused backend tests for profile, chapter granularity, migration, small-summary-only, paragraph index, trigger scan pipeline, reports, exports, and API routes.
- [ ] 13.2 Run `pytest` for the full backend test suite and fix regressions.
- [ ] 13.3 Run `npm run typecheck` from `frontend/` and fix TypeScript errors.
- [ ] 13.4 Run the WebUI locally and manually verify a precise-mode scan on a small fixture project.
- [ ] 13.5 Manually verify hybrid mode prechecks for no summaries, partial summaries, and full summaries.
- [ ] 13.6 Manually verify legacy grouped project migration and `summary_batch_size` preservation.
- [ ] 13.7 Manually verify report history, spoiler switching, context viewing, review status updates, skip-list export, and Markdown/JSON report export.
- [ ] 13.8 Document any remaining known limitations before marking the change ready for implementation review.
- [ ] 13.9 Commit final verification documentation or cleanup changes after the checks pass.
