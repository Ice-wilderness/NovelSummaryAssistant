...
- 当前状态：已新增 `docs/stability_audit/` 系列文档，并把 `split-trigger-scan-page`、`harden-frontend-api-upload` 和 `harden-chapter-splitting-boundaries` 归档到 `openspec/changes/archive/`；`persist-task-terminal-summaries` 已完成实现任务，等待归档。
...
- 当前 active change：`persist-task-terminal-summaries`，任务已完成，待归档。
