...
### 高风险：项目删除依赖目录名判断，缺少 ownership 标记
...
- 风险级别：高。
- 建议：只删除由系统创建且带 ownership/metadata 标记的导出目录；自定义目录默认不级联删除。
...
- 风险级别：中。
- 建议：限制单文件字段长度、按任务保留最近 N 条、提供清理入口。
...
- 为自定义输出目录引入 ownership/managed 标记。
- 为配置损坏恢复统一备份策略。
- 为诊断日志增加保留策略和用户可见清理动作。
